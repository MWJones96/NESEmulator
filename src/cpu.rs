...
impl CPU {
    ...
    fn fetch_addr_mode(&mut self, opcode: u8, bus: &impl CPUBus) -> AddrModeResult {
        match opcode {
            ...
            0x61 | 0x21 | 0xC1 | 0x41 | 0xA1 | 0x01 | 0xE1 | 0x81 | 0xC3 | 0xE3 => {
            ...
            | 0xA4 | 0x46 | 0x05 | 0x26 | 0x66 | 0xE5 | 0x85 | 0x86 | 0x84 | 0xC7 | 0xE7 => {
                let addr = self.fetch_byte(bus);
                self.zp(addr, bus)
            }
            0x71 | 0x31 | 0xD1 | 0x51 | 0xB1 | 0x11 | 0xF1 | 0x91 | 0xD3 | 0xF3 => {
                let addr = self.fetch_byte(bus);
                self.indy(addr, bus)
            }
            0x75 | 0x35 | 0x16 | 0xD5 | 0xD6 | 0x55 | 0xF6 | 0xB5 | 0xB4 | 0x56 | 0x15 | 0x36
            | 0x76 | 0xF5 | 0x95 | 0x94 | 0xD7 | 0xF7 => {
                let addr = self.fetch_byte(bus);
                self.zpx(addr, bus)
            }
            0xB6 | 0x96 => {
